...
export default function ProductDetails({ id }) {
  ...
  const handleDelete = async () => {
    ...
    setBusy(true);
    try { await deleteProduct(id); router.push("/dashboard"); }
  ...
  if (!product) return <div className="page-width standard-page"><div className="empty-state"><span className="empty-icon"><Icon name="search" size={32}/></span><h1 className="text-xl font-bold">This find isn't here right now</h1><p>{error}</p><Link href="/" className="button button-primary">Back to the marketplace</Link></div></div>;
  return <div className="page-width standard-page">
    <div className="breadcrumb"><Link href="/">Marketplace</Link><Icon name="chevron" size={12}/><span>{product.category}</span><Icon name="chevron" size={12}/><span>{product.title}</span></div>
